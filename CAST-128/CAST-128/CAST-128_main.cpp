...
void doCipher(int round, BYTE plaintext[8], BYTE key[16], BYTE* ciphertext){
	printf("substitionBoxes başlıyor...\n");

	InitSubstitutionBoxes();
	printf("substitionBoxes bitti!!\n");

	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	const int ROUND = 3;
	UINT32 L[ROUND + 1];
	UINT32 R[ROUND + 1];
	// INPUT: plaintext m1...m64;73bfce86



	// key K = k1...k128.;

	//BYTE plaintext[8] = { 0x48, 0x98, 0xc5, 0x88, 0xfc,0x22,0xcf,0xf8};
	...
	// 32-bit halves L0 = m1...m32 and R0 = m33...m64.);
		printf("byte82uint32LR basliyor!\n");

	byte82uint32LR(plaintext, L[0], R[0]);
		printf("byte82uint32LR bitti!\n");
	...
	//~~~~~~~~~~~~~~~
		printf("uint32LR2byte8 basliyor!\n");

	uint32LR2byte8(ciphertext, R[3], L[3]);
		printf("uint32LR2byte8 bitti!\n");

	printf("ciphertext in cipher: ");
	for (int i = 0; i < 8; ++i) {
